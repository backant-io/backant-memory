...
export async function procedurePropose(deps: ProcedureProposeDeps): Promise<{ id: string }> {
  ...
  // Repo-scoped id + retry on PK conflict (kairos#28): the owner-namespace db
  // holds many repos and memory.id is global.
  const id = await insertWithFreshLtmId(deps.db, repo, "procedure", async (id) => {
    await deps.db.batch([
      {
